...
import com.rs.game.npc.custom.PartyDemon;
import com.rs.game.npc.custom.Sunfreet;
import com.rs.game.npc.custom.ThunderDemon;
...
public final class World {
	...
	private static void sendRandomNews() {
        CoresManager.slowExecutor.scheduleWithFixedDelay(new Runnable() {
            @Override
            public void run() {
               sendWorldMessage("<col=FF4400><img=7>[Server] "+PlayerMain.getLatestNews()+"", false);
            }
	...
	public static boolean enduringRaid = false;
	...
	public static final NPC spawnNPC(int id, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea, boolean spawned) {
		...
		if (hunterNPCs != null) {
			...
			n = new ThunderDemon(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		else
	...
	public static final void spawnTemporaryObject(final WorldObject object, long time, final boolean clip) {
		...
		final WorldObject realObject = realMapObject == null ? null
				: new WorldObject(realMapObject.getId(), realMapObject.getType(), realMapObject.getRotation(), object.getX(), object.getY(), object.getPlane());
	...
	public static final boolean isMultiArea(WorldTile tile) {
		...
		return (destX >= 3462 && destX <= 3511 && destY >= 9481
				...
				|| (destX >= 3195 && destX <= 3327 && destY >= 3520 && destY <= 3970 || (destX >= 2376 && 5127 >= destY && destX <= 2422 && 5168 <= destY))
				|| (destX >= 2374 && destY >= 5129 && destX <= 2424 && destY <= 5168) // pits
				|| (destX >= 2622 && destY >= 5696 && destX <= 2573 && destY <= 5752) // torms
				|| (destX >= 2368 && destY >= 3072 && destX <= 2431 && destY <= 3135) // castlewars out
	...
	public static void sendWorldMessage(Player player, String message, boolean forStaff) {
		for (Player p : World.getPlayers()) {
			if (p == null || player == p || !p.isRunning() || p.isYellOff() || (forStaff && p.getRights() == 0))
				continue;
			p.getPackets().sendGameMessage(message);
		}
	}
