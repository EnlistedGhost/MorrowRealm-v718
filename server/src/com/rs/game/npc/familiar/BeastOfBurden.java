...
public class BeastOfBurden implements Serializable {
	...
	public void removeItem(int slot, int amount) {
		...
		if (item == null)
			return;
		if (!containsItem(item.getId(), amount))
			return;
		Item[] itemsBefore = beastItems.getItemsCopy();
	...
	public void addItem(int slot, int amount) {
		...
		if (item == null) {
			System.out.println(player.getDisplayName()+" has tried to add a null item to their BoB.");
			return;
		}
	...
	public boolean containsItem(int id, int amount) {
		if (beastItems.getNumberOf(id) >= amount) {
			return true;
		}
		return false;
	}
