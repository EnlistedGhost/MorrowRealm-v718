...
public class LucienCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
			...
			for (Entity t : possibleTargets) {
				if (t instanceof Player) {
					...
					if (!p.getMusicsManager().hasMusic(1008)) {
						...
						p.getPackets().sendGameMessage("You've received a reward while fighting Lucien!");
					}
				}
					...
					for (int[] tile : tiles.values()) {
						...
						for (Entity t : possibleTargets)
							if (t.getX() == tile[0] && t.getY() == tile[1])
								t.applyHit(new Hit(npc, Utils.random(defs.getMaxHit()+100) , HitLook.REGULAR_DAMAGE));
					}
					stop();
				}
		...
		if (attackStyle == 0) { // normal mage move
			...
			delayHit(npc, 2, target, getMagicHit(npc, getRandomMaxHit(npc, Utils.random(defs.getMaxHit()), NPCCombatDefinitions.MAGE, target)));
			World.sendProjectile(npc, target, 2963, 34, 16, 40, 35, 16, 0);
		} else if (attackStyle == 1) { // normal mage move
			npc.setNextAnimation(new Animation(11338));
			delayHit(npc, 2, target, getRangeHit(npc, getRandomMaxHit(npc, Utils.random(defs.getMaxHit()), NPCCombatDefinitions.RANGE, target)));
			...
			delayHit(npc, 4, target, getMagicHit(npc, getRandomMaxHit(npc, Utils.random(defs.getMaxHit()), NPCCombatDefinitions.MAGE, target)));
			...
			delayHit( npc, 2, target, getRegularHit(npc, Utils.random(defs.getMaxHit())));
