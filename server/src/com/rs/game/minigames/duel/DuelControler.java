...
public class DuelControler extends Controler {
	...
	public boolean canPlayerOption1(final Player target) {
		...
		if (target.getAttributes().get("DuelChallenged") == player) {
			...
			player.getControlerManager().startControler("DuelArena", target,
					target.getAttributes().get("DuelFriendly"));
			target.getControlerManager().startControler("DuelArena", player,
					target.getAttributes().remove("DuelFriendly"));
			return false;
		}
	...
	public static void challenge(Player player) {
		...
		Boolean friendly = (Boolean) player.getAttributes().remove(
				"WillDuelFriendly");
		if (friendly == null)
			return;
		Player target = (Player) player.getAttributes().remove(
				"DuelTarget");
		if (target == null
				|| target.hasFinished()
