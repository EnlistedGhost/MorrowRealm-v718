...
public class DuelArena extends Controler {
	...
	private void accept(boolean firstStage) {
		if (!hasTarget())
			return;
		boolean accepted = (Boolean) player.getAttributes().get(
				"acceptedDuel");
		boolean targetAccepted = (Boolean) target.getAttributes()
				.get("acceptedDuel");
	...
	public void cancelAccepted() {
		...
		if ((Boolean) player.getAttributes().get("acceptedDuel")) {
			player.getAttributes().put("acceptedDuel", false);
			canceled = true;
		}
		if ((Boolean) target.getAttributes().get("acceptedDuel")) {
