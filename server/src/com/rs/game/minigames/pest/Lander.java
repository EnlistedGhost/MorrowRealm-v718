...
public class Lander {
	...
	public class LobbyTimer extends TimerTask {
		...
		public void run() {
			try {
				synchronized (lock) {
					if (lobby.size() >= 1 && minutes == 1 || lobby.size() >= 1)// just
																				// for
																				// testing
						passPlayersToGame();
	...
	private void passPlayersToGame() {
		...
		synchronized (lock) {
			for (int index = 0; index < lobby.size(); index++) {
				...
				player.getControlerManager().startControler("PestControlGame",
						new PestControl(playing));
				remove(player);
			}
		}
	...
	public void enterLander(Player player) {
		if (player.getSkills().getCombatLevel() < landerRequirement
				.getRequirement()) {
			player.getDialogueManager().startDialogue(
					"SimpleMessage",
					"You need a combat level of "
							+ landerRequirement.getRequirement()
	...
	public static enum LanderRequirement {
		...
		private LanderRequirement(int id, int requirement, WorldTile tile,
				WorldTile exit, int[] pests, int reward) {
			this.id = id;
