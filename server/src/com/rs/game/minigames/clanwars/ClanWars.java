...
public final class ClanWars implements Serializable {
	...
	public static void sendConfig(Player player, Player other, int configId,
		...
		if (player.getAttributes().get("accepted_war_terms") == Boolean.TRUE) {
			player.getAttributes().remove("accepted_war_terms");
			resetAccept = true;
		}
		if (other.getAttributes().get("accepted_war_terms") == Boolean.TRUE) {
			other.getAttributes().remove("accepted_war_terms");
			resetAccept = true;
		}
		if (resetAccept) {
	...
	public void sendInterface(Player p, Player other) {
		p.getAttributes().put("clan_wars", this);
	...
	public void accept(final Player player) {
		final Player other = (Player) player.getAttributes().get(
				"clan_request_p");
		if (other != null
				&& (Boolean) other.getAttributes().get(
						"accepted_war_terms") == Boolean.TRUE) {
			CoresManager.slowExecutor.submit(new Runnable() {
				@Override
				public void run() {
					player.getAttributes().remove(
							"accepted_war_terms");
					other.getAttributes()
		...
		player.getAttributes().put("accepted_war_terms", true);
	}
	...
	public static void enter(Player p) {
		...
		final ClanWars c = hasWar ? p.getCurrentFriendChat().getClanWars()
				: (ClanWars) p.getAttributes().get("view_clan");
		if (c == null) {
			return;
		}
		...
		if (hasWar && c.timer.isStarted() && c.isKnockOut()) {
			...
			p.getAttributes().put("view_prefix",
		...
		Integer prefix = (Integer) p.getAttributes().get(
				"view_prefix");
