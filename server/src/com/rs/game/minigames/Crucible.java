...
public class Crucible {
	...
	public static boolean isImmune(Player player) {
		Long immune = (Long) player.getAttributes().get(
				"CrucibleImmune");
	...
	public static void setImmune(Player player, int seconds) {
		if (seconds == 0)
			player.getAttributes().remove("CrucibleImmune");
		else
			player.getAttributes().put("CrucibleImmune",
	...
	public static void openFissureTravel(Player player) {
		...
		player.getAttributes().remove("crucibleBounty");
	}
	...
	public static void payBountyFee(Player player, CrucibleControler controler) {
		Fissures fissure = (Fissures) player.getAttributes().remove(
				"crucibleBounty");
		if (fissure == null)
			return;
	...
	public static void travel(Player player, Fissures fissure,
		...
		if (!isInside) {
			if (isBankFissure(fissure))
				...
				player.getAttributes().put("crucibleBounty", fissure);
			}
		} else {
			travel(player, fissure.tile);
			if (isBankFissure(fissure))
