...
public final class RegionBuilder {
	...
	public static void destroyAllPlanesMap(int toRegionX, int toRegionY, int ratio) {
		int[] planes = new int[4];
		for(int plane = 1; plane < 4; plane++)
			planes[plane] = plane;
		destroyMap(toRegionX, toRegionY, ratio, ratio, planes, planes);
	}
	
	public static void destroyMap(int toRegionX, int toRegionY, int widthRegions, int heightRegions, int[] fromPlanes, int[] toPlanes) {
		...
		for(int xOffset = 0; xOffset < widthRegions; xOffset++) {
			for(int yOffset = 0; yOffset < heightRegions; yOffset++) {
				...
				//System.out.println("toX: "+toThisRegionX+" toY: "+toThisRegionY);
				//System.out.println("<<8X: "+((toThisRegionX / 8) << 8)+" Y: "+(toThisRegionY / 8)+" both: "+(((toThisRegionX / 8) << 8) + (toThisRegionY / 8)) );
				...
				//System.out.println("RegOffX: "+regionOffsetX+" RegOffY: "+regionOffsetY);
