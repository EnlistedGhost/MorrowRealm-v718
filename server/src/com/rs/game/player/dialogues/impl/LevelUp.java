...
public final class LevelUp extends Dialogue {
	...
	public void start() {
		...
		player.getAttributes().put("leveledUp", skill);
		player.getAttributes().put("leveledUp[" + skill + "]", Boolean.TRUE);
	...
	public static void sendNews(Player player, int skill, int level) {
		...
		if (!reachedAll) {
			for (Player plr : World.getPlayers()) {
				plr.getPackets().sendSound(6, 0, 2);
			}
			World.sendWorldMessage("<img=7><col=ff8c38>News: " + player.getDisplayName() + " has achieved " + level + " " + Skills.SKILLS[skill] + ".", false);
			return;
		}
		for (Player plr : World.getPlayers()) {
			plr.getPackets().sendSound(6, 0, 2);
