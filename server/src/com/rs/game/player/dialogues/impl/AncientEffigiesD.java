	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			sendOptionsDialogue("Which images do you wish to focus on?",
					Skills.SKILLS[skill1], Skills.SKILLS[skill2]);
			stage = 1;
		} else if (stage == 1 && componentId == 2) {
			if (player.getSkills().getLevel(
					(Integer) player.getAttributes().get("skill1")) < AncientEffigies
			...
			if (player.getSkills().getLevel(
					(Integer) player.getAttributes().get("skill2")) < AncientEffigies
					.getRequiredLevel(itemId)) {
				sendDialogue(
