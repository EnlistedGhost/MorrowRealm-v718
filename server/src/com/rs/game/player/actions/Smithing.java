...
public class Smithing extends Action {
	...
	public boolean process(Player player) {
		...
		if (!player.getTools().contains(HAMMER)) {
	...
	public int processWithDelay(Player player) {
		ticks--;
			if (player.isUsingAltEmotes() == true) {
	...
	public boolean start(Player player) {
		if ((bar = (ForgingBar) player.getAttributes().get("SmithingBar")) == null) {
			return false;
		}
		...
		if (!player.getInventory().containsItem(bar.getBarId())) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You have no bars to smelt!");
			return false;
		}
	...
	public static class ForgingInterface {
		...
		public static void sendSmithingInterface(Player player, ForgingBar bar) {
			...
			player.getAttributes().put("SmithingBar", bar);
