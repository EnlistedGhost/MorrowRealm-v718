...
public class PlayerCombat extends Action {
	...
	public int processWithDelay(Player player) {
		...
		if (player.getAttributes().get("miasmic_effect") == Boolean.TRUE) 
			multiplier = 1.5;
		...
		if (spellId > 0) {
			...
			if (isRanging == 0) {
				int attackDelay = (int) (meleeAttack(player) * multiplier);
				return attackDelay;
			} else if (isRanging == 1) {
	...
	public int mageAttack(final Player player, int spellId, boolean autocast) {
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 36: //Miasmic rush
				...
				if (target.getAttributes().get("miasmic_immunity") == Boolean.TRUE) {
					return 4;
				}
				if (target instanceof Player) {
					((Player) target).getPackets().sendGameMessage("You feel slowed down.");
				}
				target.getAttributes().put("miasmic_immunity", Boolean.TRUE);
				target.getAttributes().put("miasmic_effect", Boolean.TRUE);
				final Entity t = target;
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						t.getAttributes().remove("miasmic_effect");
						WorldTasksManager.schedule(new WorldTask() {
							@Override
							public void run() {
								t.getAttributes().remove("miasmic_immunity");
								stop();
							}
						}, 15);
						stop();
			...
			case 37: //Miasmic blitz
				...
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						t0.getAttributes().remove("miasmic_effect");
						WorldTasksManager.schedule(new WorldTask() {
							@Override
							public void run() {
								t0.getAttributes().remove("miasmic_immunity");
								stop();
							}
						}, 15);
						stop();
			...
			case 23: // ice barrage
				...
				attackTarget(getMultiAttackTargets(player, 9, 4), new MultiAttack() {
	...
	public int getArrowProjectileGfxId(int weaponId, int arrowId) {
		if (arrowId == 884) {
			...
			return 1120;
		else if (weaponId == 20171 || weaponId == 24456)
			return 1066;
	...
	private int meleeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 4151:
			case 23691:
				player.setSwitchDelay(Utils.currentTimeMillis() + 700);
			...
			case 23690:
				player.setSwitchDelay(Utils.currentTimeMillis() + 1000);
			...
			case 13774:
			case 13776:
				player.setSwitchDelay(Utils.currentTimeMillis() + 500);
			...
			case 13899: // vls
			case 13901:
				player.setSwitchDelay(Utils.currentTimeMillis() + 600);
			...
			case 14484: //d claws
			case 23695:
				player.setSwitchDelay(Utils.currentTimeMillis() + 1500);
				player.setNextAnimation(new Animation(10961));
				player.setNextGraphics(new Graphics(1950));
				int[] hits = new int[] {0, 1};
				int hit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.4, true);
				if (hit > 0) {
					hits = new int[] {hit, hit / 2, (hit / 2) / 2, (hit / 2) - ((hit / 2) / 2)};
				} else {
					hit = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.4, true);
					if (hit > 0) {
	...
	public static final boolean usingGoliathGloves(Player player) {
		String name = player.getEquipment().getItem(Equipment.SLOT_SHIELD) != null ? player
				.getEquipment().getItem(Equipment.SLOT_SHIELD).getDefinitions().getName().toLowerCase() : "";
				if (player.getEquipment().getItem((Equipment.SLOT_HANDS)) != null) {
					if (player.getEquipment().getItem(Equipment.SLOT_HANDS).getDefinitions().getName().toLowerCase().contains("goliath")
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE
						...
						if (combatXp > 0) {
							...
							if (hit.getLook() == HitLook.RANGE_DAMAGE) {
								if (attackStyle == 2) {
									player.getSkills()
									.addXp(Skills.RANGE, combatXp / 10);
		...
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.contains("chinchompa"))
					return 2779;
				if (weaponName.contains("staff of light") || weaponName.contains("obliteration")) {
					switch (attackStyle) {
					case 0:
						return 15072;
					case 1:
				...
				if (weaponName.contains("frostmourne")) {
					switch (attackStyle) {
					case 2:
						return 15072;
					default:
	...
	private boolean checkAll(Player player) {
		...
		maxDistance = isRanging != 0
				...
				player.getAttributes().put("last_target", target);
				if (target != null)
					target.getAttributes().put("last_attacker", player);
	...
	private static boolean hasPolyporeStaff(Player player) {
		int weaponId = player.getEquipment().getWeaponId();
		return weaponId == 22494 || weaponId == 22496 || weaponId == 24457;
	}
