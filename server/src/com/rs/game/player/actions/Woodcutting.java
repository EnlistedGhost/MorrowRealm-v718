...
public final class Woodcutting extends Action {
	...
	public static enum TreeDefinitions {
		...
		private TreeDefinitions(int level, double xp, int logsId,
				int logBaseTime, int logRandomTime, int stumpId, int respawnDelay, int randomLifeProbability) {
			this.level = level;
			this.xp = xp;
			this.logsId = logsId;
	...
	private boolean checkAll(Player player) {
		...
		if (!hasAxe(player) && !player.getTools().contains(1351)) {
	...
	private boolean hasWoodcuttingLevel(Player player) {
		if (definitions.getLevel() > player.getSkills().getLevel(8)) {
			player.getPackets().sendGameMessage("You need a woodcutting level of " + definitions.getLevel()
	...
	private boolean setAxe(Player player) {
		if (player.getTools().contains(1351) && !hasAxe(player)) {
			emoteId = 879;
			axeTime = 1;
			return true;
		}
	...
	private void addLog(Player player) {
		...
		player.setLogsCut(player.getLogsCut() + 1);
		checkLogsCut(player, player.getLogsCut());
	...
	public void checkLogsCut(Player player, int logsCut) {
		boolean toBank = player.getInventory().hasFreeSlots();
		
		if (logsCut == 100) {
			player.sendMessage("<col=0000FF>You have cut "+logsCut+" logs and recieved the lumberjack top!");
			if (!toBank) {
				player.getBank().addItem(10939, 1, true);
				player.sendMessage("<col=0000FF>Lumberjack Top has been sent to your bank.");
			} else {
		...
		if (logsCut == 250) {
			player.sendMessage("<col=0000FF>You have cut "+logsCut+" logs and recieved the lumberjack legs!");
			if (!toBank) {
				player.getBank().addItem(10940, 1, true);
				player.sendMessage("<col=0000FF>Lumberjack Legs has been sent to your bank.");
			} else {
		...
		if (logsCut == 500) {
			player.sendMessage("<col=0000FF>You have cut "+logsCut+" logs and recieved the lumberjack hat!");
			if (!toBank) {
				player.getBank().addItem(10941, 1, true);
				player.sendMessage("<col=0000FF>Lumberjack Hat has been sent to your bank.");
			} else {
		...
		if (logsCut == 750) {
			player.sendMessage("<col=0000FF>You have cut "+logsCut+" logs and recieved the lumberjack boots!");
			if (!toBank) {
				player.getBank().addItem(10933, 1, true);
				player.sendMessage("<col=0000FF>Lumberjack Boots has been sent to your bank.");
			} else {
