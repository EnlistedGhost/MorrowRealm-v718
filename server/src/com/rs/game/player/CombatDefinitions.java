...
public final class CombatDefinitions implements Serializable {
	...
	public static final int DAMAGEDONE = Utils.random(1, 20);
	...
	public int getSpellId() {
		Integer tempCastSpell = (Integer) player.getAttributes().get(
				"tempCastSpell");
	...
	public void resetSpells(boolean removeAutoSpell) {
		player.getAttributes().remove("tempCastSpell");
		if (removeAutoSpell) {
			setAutoCastSpell(0);
	...
	public static final int getMeleeBonusStyle(int weaponId, int attackStyle) {
		if (weaponId != -1) {
			...
			if (weaponName.contains("obliteration")) {
				return CRUSH_ATTACK;
			}
			
			if (weaponName.contains("frostmourne")) {
				switch (attackStyle) {
				case 0: return SLASH_ATTACK;
				case 1: return SLASH_ATTACK;
				case 2: return STAB_ATTACK;
				default: return SLASH_ATTACK;
				}
			}
			if (weaponName.contains("annihilation")) {
				switch (attackStyle) {
				case 0: return CRUSH_ATTACK;
				case 1: return CRUSH_ATTACK;
				case 2: return STAB_ATTACK;
				default: return CRUSH_ATTACK;
				}
			}
	...
	public static final int getXpStyle(int weaponId, int attackStyle) {
		if (weaponId != -1 && weaponId != -2) {
			...
			if (weaponName.contains("staff") || weaponName.contains("obliteration")) {
				switch (attackStyle) {
				case 0:
					return Skills.ATTACK;
				case 1:
	...
	public void restoreSpecialAttack() {
		...
		if (specialAttackPercentage == 100 || specialAttackPercentage == 50)
			player.getPackets().sendGameMessage(
					"<col=088A08>Your special attack energy is now "
