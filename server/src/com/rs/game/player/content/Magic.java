...
public class Magic {
	...
	public static final boolean checkCombatSpell(Player player, int spellId, int set, boolean delete) {
		...
		case 193:
			...
			case 36: // Miasmic rush.
				...
				if (weaponId != 13867 && weaponId != 13869 && weaponId != 13941 && weaponId != 13943 && player.getRights() == 0) {
	...
	public static final void processLunarSpell(Player player, int spellId,
		...
		case 37:
			...
			Long lastVeng = (Long) player.getAttributes().get(
					"LAST_VENG");
			if (lastVeng != null
			...
			player.getAttributes().put("LAST_VENG",
	...
	private static void useHomeTele(Player player) {
		...
		//sendNormalTeleportSpell(player, 0, 0, Settings.START_PLAYER_LOCATION);
	}
	...
	public static final boolean sendTeleportSpell(final Player player,
		...
		if (player.getCannon().hasCannon()) {
			player.getCannon().pickUpDwarfCannon(player.getCannon().getObject());
		}
