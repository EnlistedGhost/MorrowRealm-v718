...
public final class Skills implements Serializable {
	...
	public int getLevelForXp(int skill) {
		...
		int points = 0;
		int output = 0;
		for (int lvl = 1; lvl <= (skill == DUNGEONEERING ? 120 : 120); lvl++) {// Define max levels
			points += Math.floor(lvl + 300.0 * Math.pow(2.0, lvl / 7.0));
			output = (int) Math.floor(points / 4);
			if ((output - 1) >= exp) {
				return lvl;
		...
		return skill == DUNGEONEERING ? 120 : 120;// Define max levels
	}
	...
	public boolean hasMaxedCombat() {
		return hasRequiriments(ATTACK, 99, STRENGTH, 99, DEFENCE, 99, RANGE, 99, PRAYER, 99, MAGIC, 99, HITPOINTS, 99);
	}
	...
	public boolean hasMaxedSkills() {
		return hasRequiriments(COOKING, 99, WOODCUTTING, 99, FLETCHING, 99, FISHING, 99, FIREMAKING,99 ,
				...
				HUNTER, 99,  SUMMONING, 99, DUNGEONEERING, 99);
	}
