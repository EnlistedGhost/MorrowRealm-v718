...
public class Trade {
	...
	private ItemsContainer<Item> stored;
		...
		synchronized (this) {
			...
			synchronized (target.getTrade()) {
				...
				if (!ItemConstants.isTradeable(item)) {
					player.getPackets().sendGameMessage("<col=FF0000>"+itemName+" is not tradeable.");
					return;
				}
	...
	public void refreshFreeInventorySlots() {
		...
		target.getPackets().sendIComponentText(
				335, 23, "has " + (freeSlots == 0 ? "no" : freeSlots) + " free" + "<br>inventory slots");
	}
		...
		synchronized (this) {
			synchronized (target.getTrade()) {
				...
				if (oldTarget.getInventory().getItems().goesOverAmount(oldTarget.getTrade().items)) {
					oldTarget.getPackets().sendGameMessage("You'll go over max of an item!");
					player.getPackets().sendGameMessage("They'll go over max of an item!");
				...
				if (player.getInventory().getItems().goesOverAmount(items)) {
					player.getPackets().sendGameMessage("You'll go over max of an item!");
					oldTarget.getPackets().sendGameMessage("They'll go over max of an item!");
					player.setCloseInterfacesEvent(null);
					player.closeInterfaces();
