...
public class Bank implements Serializable {
	...
	public void depositMoneyPouch(boolean banking) {
		if (player.getMoneyPouch() <= 0) {
			player.sendMessage("Your money pouch is empty!");
			return;
		}
		int coinsCount = player.getMoneyPouch() > Integer.MAX_VALUE ? Integer.MAX_VALUE : (int)player.getMoneyPouch();
		...
		if (space != 0) {
			...
			player.getPackets().sendGameMessage(getFormattedNumber((int)player.getMoneyPouch()) + " coins have been removed from your money pouch.");
	...
	public void depositPouch() {
		Item item = new Item(995, player.getMoneyPouch());
		int quantity = player.getMoneyPouch();
		if (item == null)
			return;
		Item bankedItem = getItem(item.getId());
		if (bankedItem != null) {
		...
		MoneyPouch.removeMoney(item.getAmount(), player, false);
		addItem(item, true);
	}
	...
	public boolean hasItem(Item i) {
		for (int slot = 0; slot < bankTabs.length; slot++) {
			for (Item item : bankTabs[slot])
				if (item.getId() == i.getId() && item.getAmount() >= i.getAmount())
					return true;
		}
		return false;
	}
