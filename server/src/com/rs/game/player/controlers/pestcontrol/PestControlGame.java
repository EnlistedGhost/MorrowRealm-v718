...
public class PestControlGame extends Controler {
	...
	public void start() {
		...
		player.getAttributes().put("pestPoints", 0);
	...
	private void updatePestPoints() {
		player.getPackets().sendIComponentText(408, 11, "" + player.getAttributes().get("pestPoints"));
	}
	...
	public boolean keepCombating(Entity target) {
		for (Hit hit : player.getNextHits()) {
			if (hit == null)
				return false;
			player.getAttributes().put("pestPoints", hit.getDamage());
			updatePestPoints();
		}
		return true;
	}
