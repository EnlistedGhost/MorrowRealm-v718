...
public class FightCaves extends Controler {
	...
	public static void enterFightCaves(Player player) {
		if (player.getInventory().containsItem(6, 1)
				|| player.getInventory().containsItem(8, 1)
				|| player.getInventory().containsItem(10, 1)
				|| player.getInventory().containsItem(12, 1)) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", THHAAR_MEJ_JAL, "Cannons are not allowed in the fight caves!");
			return;
		}
			...
			if (type == 1 || type == 4) {
				...
				if (type == 4) {
					for (Player players : World.getPlayers()) {
						if (players == null)
							continue;
						players.getPackets().sendGameMessage("<img=7><col=ff8c38>News: " + player.getDisplayName() + " Just completed Fightcaves.");
					}
					...
					if (!player.getInventory().addItem(6570, 1) && player.getInventory().addItem(23659, 1)) {
						...
						World.addGroundItem(new Item(23659, 1), new WorldTile(player), player, true, 180, true);
