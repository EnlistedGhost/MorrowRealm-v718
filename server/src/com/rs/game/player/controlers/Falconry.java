...
public class Falconry extends Controler {
	...
	public boolean processNPCClick1(final NPC npc) {
		...
		if (npc.getDefinitions().name.toLowerCase().contains("kebbit")) {
			if (player.getAttributes().get("falconReleased") != null) {
				player.getDialogueManager().startDialogue("SimpleMessage",
			...
			if (proccessFalconAttack(npc)) {
				...
				"gyr falcon")) {
			NPC kill = (NPC) player.getAttributes()
					.get("ownedFalcon");
			if (kill == null)
				return false;
			if (kill != npc) {
			...
			player.getEquipment().refresh(3);
			player.getAppearence().generateAppearenceData();
			player.getAttributes().remove("ownedFalcon");
			player.getAttributes().remove("falconReleased");
			return true;
		}
		return true;
	}
