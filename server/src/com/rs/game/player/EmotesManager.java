...
public final class EmotesManager implements Serializable {
	...
	public void useBookEmote(int id, int slotId) {
		...
		if (!unlockedEmotes.contains(id)) {
			...
			if (slotId == 0) {
				...
				player.setNextAnimation(new Animation(0x3172));
				player.setNextGraphics(new Graphics(780));
			} else if (slotId == 49) {
				player.setNextAnimation(new Animation(0x3ABA));
				player.setNextGraphics(new Graphics(2930));
			} else if (slotId == 48) {
				player.setNextAnimation(new Animation(0x3A15));
	        	...
	        	final NPC n = new NPC(14388, new WorldTile(player.getX(), player.getY() + 4, player.getPlane()), 0, false);
	        	n.setLocation(n);
			...
			setNextEmoteEnd();
			ClueScrolls.completedRiddle(player, slotId);
		}
	}
