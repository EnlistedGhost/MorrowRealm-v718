	...
	private static final void packObjectSpawns() {
		...
		try {
			...
			while (true) {
				...
				ObjectDefinitions defs = ObjectDefinitions.getObjectDefinitions(objectId);
				if (defs.name == "null" || defs.name == null) {
					System.out.println("Stopped Nulled Object from Spawning..."+objectId+"");
					continue;
				}
				addObjectSpawn(objectId, type, rotation, tile.getRegionId(), tile, Boolean.parseBoolean(splitedLine3[3]));
				System.out.println("Spawned Object..."+objectId+"");
			}
			in.close();
		} catch (Throwable e) {
			Logger.handle(e);
